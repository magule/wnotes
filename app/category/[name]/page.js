...
export default function CategoryPage({ params }) {
  ...
  const categoryName = params ? decodeURIComponent(params.name) : '';
  ...
  useEffect(() => {
    ...
    if (savedNotes) {
      ...
      // Format dates using our consistent formatter
      const dates = {};
      filteredNotes.forEach(note => {
        dates[note.id] = formatDate(note.updatedAt);
      });
  ...
  const handleDeleteCategory = () => {
    if (confirm("This will delete all the notes inside this category. Are you sure?")) {
      ...
      if (savedNotes && savedCategories) {
        ...
        // Filter out notes from this category
        const updatedNotes = allNotes.filter(note => note.tag !== categoryName);
  ...
  const handleBulkExport = async (format) => {
    try {
      ...
      a.href = url;
      a.download = `${categoryName}_notes.${extension}`;
    ...
    try {
      switch (format) {
        ...
        case 'pdf':
          ...
          pdf.setFontSize(24);
          pdf.setFont("helvetica", "bold");
          pdf.text(note.title || 'Untitled', 20, 20);
          pdf.setFontSize(12);
          pdf.setFont("helvetica", "normal");
          const textLines = pdf.splitTextToSize(note.content, 170);
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-10">
            ...
            <div>
              ...
              <div className="space-y-4 text-sm">
                <p className="text-white/70">
                  Here are all your notes tagged with "{categoryName}". You can:
                </p>
                <ul className="space-y-2 text-white/60">
                  ...
                  <li className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-400/40"></div>
                    View and manage all your notes in "My Notes"
                  </li>
                </ul>
              </div>
            </div>
        ...
        <div className="mb-8 flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-pink-400">
            Notes in {categoryName} <span className="text-white/40 text-lg">({notes.length})</span>
          </h1>
          <button
            onClick={() => router.push('/')}
            className="px-4 py-2 rounded-lg bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 
          ...
          {notes.length === 0 ? (
            <div className="col-span-full text-center py-20">
              <div className="inline-flex flex-col items-center gap-4">
                ...
                <div className="text-white/40">No notes in this category yet</div>
                <button
                  onClick={() => router.push('/')}
                  className="px-4 py-2 rounded-lg bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 
                           transition-all duration-200 text-sm font-medium flex items-center gap-2"
